// moved from sample/pattern_2_list/tab-stock-reg
"use client"
...
export default function StockRegPage() {
  ...
  const derived = useMemo(() => {
    ...
    let elapsed: string = '-'
    if (dateStr) { const dt = new Date(dateStr); if (!isNaN(+dt)) { const today = new Date(); const days = Math.max(0, Math.round((+today - +dt) / 86400000)); elapsed = `${days}日` } }
  ...
  async function handleSubmit(ev: React.FormEvent<HTMLFormElement>) {
    ev.preventDefault(); setMsg('保存中...'); setSubmitting(true)
    try {
          ...
          <form onSubmit={(ev) => { handleSubmit(ev).catch(console.error) }} className="space-y-6">
              ...
              <div className="grid md:grid-cols-3 gap-4 text-sm">
                <label className="block">親（過去成約）
                  <select className="mt-1 w-full border rounded-lg px-3 py-2" value={form.estate_entry_id} onChange={onChange('estate_entry_id')} required>
                    ...
                    {entries.map(e => (<option key={e.id} value={e.id}>{e.estate_name ?? '(名称未設定)'}（{e.id.slice(0,8)}…）</option>))}
                  </select>
                </label>
                ...
                <div className="block"><div className="text-gray-500">登録からの経過</div><div className="mt-1">{derived.elapsed}</div></div>
              </div>
            </section>
            ...
            <section className="space-y-4">
              ...
              <div className="grid md:grid-cols-3 gap-4 text-sm">
                ...
                <label className="block md:col-span-3">マイソクPDF
                  <input id="mysoku" type="file" accept="application/pdf" className="mt-1 w-full border rounded-lg px-3 py-2" onChange={(e) => setPdf(e.target.files?.[0] ?? null)} />
                </label>
            ...
            <div className="flex items-center justify-end gap-2">
              <button type="reset" className="px-3 py-1.5 rounded-lg bg-gray-100 text-sm" onClick={() => { setForm({ estate_entry_id: '', floor: '', area_sqm: '', list_price: '', registered_date: '', broker_name: '', broker_pref: '', broker_city: '', broker_town: '', broker_tel: '', broker_person: '', broker_mobile: '', broker_email: '', broker_mysoku_url: '', broker_photo_url: '', fundplan_url: '', status: '問い合わせ' }); setPdf(null); setMsg('') }}>リセット</button>
