...
import { calcComplexMonthlyCostTotal, parseMonthlyAmount } from '@/lib/complexForm'
...
type RentCaseAvailability = '' | '有' | '無し'
...
export default function TabComplexPage() {
  ...
  async function handleSubmit(ev: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setEvalForm(initialEval)
      setBuiltAge(null)
      router.push('/tab-list')
    } catch (e: unknown) {
      console.error('[complex/save]', e)
      ...
      <header className="bg-white border-b sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold">団地基本情報 + 評価シート</h1>
          </div>
        ...
        <section id="tab-complex" className="tab active">
          <div className="bg-white rounded-2xl shadow p-5 space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold">団地基本情報 + 評価シート</h2>
              <div className="flex items-center gap-2 text-sm">
                <span className="text-gray-500">{msg || '入力後、保存してください'}</span>
              </div>
            </div>
